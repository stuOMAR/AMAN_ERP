...
from utils.currency_display import currency_amount_base_sql, document_amount_base_sql
...
def get_sales_kpis(db, start_date: date, end_date: date,
    ...
    if overdue_count > 0:
        alerts.append({"severity": "high", "code": "OVERDUE_INVOICES",
                        "message": f"{overdue_count} invoices overdue totaling {overdue_value:,.0f} {base_currency}",
