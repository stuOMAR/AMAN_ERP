  ...
  const { showToast } = useToast()
    ...
    const fetchList = useCallback(async () => {
        try {
            ...
            const res = await salesAPI.listCreditNotes(params)
    ...
    const handleCreate = async () => {
        ...
        if (!form.lines.length || !hasPositiveLine(form.lines)) return showToast(t('sales.credit_notes.at_least_one_item', 'info'))
        try {
            setSaving(true)
    ...
    const viewDetail = async (id) => {
        try {
            const res = await salesAPI.getCreditNote(id)
        ...
        <div className="workspace fade-in">
            ...
            <div className="workspace-header">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                    <div>
                        <h1 className="workspace-title">📋 {t('sales.credit_notes.title')}</h1>
                        <p className="workspace-subtitle">{t('sales.credit_notes.subtitle')}</p>
                    </div>
                    <button className="btn btn-primary" onClick={openCreate}>+ {t('sales.credit_notes.create')}</button>
                </div>
            </div>

            {/* Filters */}
            <div className="card" style={{ padding: 16, display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
                <input className="form-input" placeholder={t('sales.credit_notes.search_placeholder')} value={search}
            ...
            <div className="card card-flush" style={{ overflow: 'auto' }}>
                <table className="data-table">
                    <thead>
                        <tr>
                            <th>{t('sales.credit_notes.note_number')}</th>
                            <th>{t('common.customer')}</th>
                            <th>{t('common.date')}</th>
                            <th>{t('sales.credit_notes.related_invoice')}</th>
                    ...
                    <tbody>
                        {items.length === 0 ? (
                            <tr><td colSpan="6" style={{ textAlign: 'center', padding: 40 }}>{t('sales.credit_notes.no_credit_notes')}</td></tr>
                        ) : items.map(item => (
                ...
                <div className="modal-overlay" onClick={() => setShowCreate(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 1200, width: '95%', maxHeight: '95vh', overflow: 'auto' }}>
                        <div className="modal-header">
                            <h2>{t('sales.credit_notes.create_title')}</h2>
                        ...
                        <div style={{ padding: 20 }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                                <div>
                                    ...
                                    <select className="form-input" value={form.party_id} onChange={e => {
                                        ...
                                        <option value="">{t('sales.credit_notes.choose_customer')}</option>
                                        {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="form-label">{t('sales.credit_notes.related_invoice')}</label>
                                    <select className="form-input" value={form.related_invoice_id} onChange={e => setForm(f => ({ ...f, related_invoice_id: e.target.value }))}>
                                        <option value="">{t('sales.credit_notes.no_link')}</option>
                            ...
                            {/* Lines */}
                            <h3 style={{ marginBottom: 12 }}>{t('sales.credit_notes.items')}</h3>
                                ...
                                <table className="data-table">
                                    <thead style={{ background: 'var(--bg-secondary)' }}>
                                        <tr>
                                            ...
                                            <th style={{ width: '10%' }}>{t('sales.credit_notes.tax_percent')}</th>
                                            ...
                                            <tr key={i}>
                                                <td>
                                                    <select className="form-input" value={line.product_id || ''} onChange={e => updateLine(i, 'product_id', e.target.value)}>
                                                        <option value="">-- {t('sales.credit_notes.choose_product')} --</option>
                            ...
                            <button className="btn btn-secondary" style={{ marginTop: 8 }} onClick={addLine}>+ {t('sales.credit_notes.add_item')}</button>
                            ...
                            <div style={{ marginTop: 20, display: 'flex', justifyContent: 'flex-end' }}>
                                <div style={{ minWidth: 250, background: 'var(--card-bg)', padding: 16, borderRadius: 8 }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                                        <span>{t('sales.credit_notes.tax')}:</span><strong>{backendTotals?.totalTax != null ? formatNumber(backendTotals.totalTax) : '—'} {currency}</strong>
                                    </div>
                                    <hr />
                            ...
                            <div style={{ marginTop: 24, display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                                ...
                                <button className="btn btn-primary" onClick={handleCreate} disabled={saving}>
                                    {saving ? t('sales.credit_notes.saving') : t('sales.credit_notes.create')}
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            {showDetail && detailItem && (
                <div className="modal-overlay" onClick={() => setShowDetail(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 800, maxHeight: '90vh', overflow: 'auto' }}>
                        <div className="modal-header">
                            <h2>{t('sales.credit_notes.detail_title')}: {detailItem.invoice_number}</h2>
                        ...
                        <div style={{ padding: 20 }}>
                            ...
                            {detailItem.related_invoice_number && (
                                <p style={{ marginBottom: 12 }}>🔗 {t('sales.credit_notes.linked_to_invoice')}: <strong>{detailItem.related_invoice_number}</strong></p>
                            )}
                            {detailItem.notes && <p style={{ marginBottom: 16, color: '#666' }}>📝 {detailItem.notes}</p>}
                            <table className="data-table">
                                <thead>
                                    <tr><th>{t('common.description')}</th><th>{t('common.quantity')}</th><th>{t('common.price')}</th><th>{t('sales.credit_notes.tax_percent')}</th><th>{t('common.discount')}</th><th>{t('common.total')}</th></tr>
                                </thead>
                                <tbody>
                            ...
                            <div style={{ marginTop: 16, textAlign: 'left', display: 'flex', justifyContent: 'flex-end' }}>
                                <div style={{ minWidth: 200 }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>{t('sales.credit_notes.tax')}:</span><span>{formatNumber(detailItem.tax_amount)}</span></div>
                                    <hr />
